...
import { CheckCircle2, Mic, Stethoscope } from "lucide-react";
import GenericHero from "@/components/GenericHero";
import CTABox from "@/components/CTABox";

export default function ClinicalDocumentationContent() {
  const services = [
    {
      icon: Mic,
      title: "Medical Transcription",
      description:
        "Our experienced clinical documentation editors (CDEs) transcribe physician dictations into accurate, formatted medical records. From routine office visits to complex surgical reports, we handle all specialties with 99.5% accuracy and same-day turnaround.",
    ...
    },
    {
      ...
      description:
        "Dedicated scribes document the clinical encounter in real time, allowing physicians to focus on patient care. Our scribing team is trained across specialties and integrated into your existing EHR workflow.",
  ...
  const workflowSteps = [
    {
      ...
      description:
        "Physician dictations are securely received via our HIPAA-compliant platform — supporting all audio formats, EHR integrations, and mobile dictation devices.",
    },
    {
      number: "02",
      title: "AI-Enhanced Processing",
      description:
        "Our AI engine performs initial speech-to-text conversion with medical vocabulary, generating a draft that is 85-90% accurate before human review begins.",
    },
    {
      number: "03",
      title: "Expert Editing",
      description:
        "Skilled CDEs review and perfect every transcript against the original audio — correcting terminology, formatting per client specifications, and applying our Book of Style standards.",
    },
    {
      number: "04",
      title: "Quality Assurance",
      description:
        "Senior QA auditors perform word-for-word verification against the audio, ensuring 99.5%+ accuracy before the document reaches the patient's permanent record.",
    },
  ];
        ...
        <GenericHero
          title={
            <>
              ...
              <span className="block text-[#2563eb]">You Can Trust</span>
            </>
          }
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {services.map((service, idx) => (
              ...
              >
                ...
                <h3 style={{ fontWeight: 500, fontSize: "24px" }} className="text-slate-900 mb-3">
                  {service.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {service.features.map((f) => (
                    <span key={f} style={{ fontWeight: 500, fontSize: "13px" }} className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg">
                      {f}
                    </span>
                  ))}
                </div>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {specialties.map((s) => (
              <span
                ...
                className="px-4 py-2 bg-white rounded-xl border border-gray-200 text-slate-700 hover:border-blue-300 hover:text-blue-700 transition-colors"
              >
                {s}
              </span>
            ))}
