    ...
    <section className={`relative flex items-center overflow-hidden bg-gradient-to-br from-white via-blue-50/30 to-white ${isSmall ? 'min-h-[60vh]' : 'min-h-[80vh]'}`}>
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8 py-20 w-full">
        <div className={image ? "grid lg:grid-cols-2 gap-12 items-center" : ""}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className={image ? "" : "max-w-3xl"}>
            {subtitle && (
              <motion.div
            ...
            <h1 style={{ fontWeight: 800, fontSize: image ? '52px' : '64px', letterSpacing: '-0.03em', lineHeight: '1.2' }} className="text-slate-900 mb-6">
              {title}
            </h1>
