...
const fontStyles = `
  ...
  * {
  ...
  h2 { font-weight: 700; font-size: 36px; line-height: 1.3; }
  h3 { font-weight: 700; line-height: 1.3; }
  body, p { font-weight: 400; font-size: 18px; line-height: 1.6; }
  small, caption, [data-caption] { font-weight: 500; font-size: 14px; line-height: 1.5; }
  button, .button { font-weight: 700; text-transform: none; }
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <Link to={createPageUrl('Home')} onClick={() => window.scrollTo(0, 0)} className="flex items-center gap-2">
                  ...
                  {item.page ? (
                    <Link
                      ...
                      className={`flex items-center gap-1 transition-colors ${
                        currentPageName === item.page ? 'text-[#2563eb]' : 'text-gray-700 hover:text-[#2563eb]'
                      }`}
                    >
                      {item.name}
                    </Link>
            ...
            >
              <div className="px-6 py-4 space-y-2">
                {navigation.map((item) => (
                  <div key={item.name}>
                    {item.page ? (
                      <Link to={createPageUrl(item.page)} onClick={() => setMobileMenuOpen(false)}
                        className={`block py-3 text-lg font-medium ${currentPageName === item.page ? 'text-[#2563eb]' : 'text-gray-700'}`}>
                        {item.name}
                      </Link>
                    ) : (
                      <button className="block py-3 text-lg font-medium text-gray-700 w-full text-left">{item.name}</button>
                    )}
                    {item.children && (
                      <div className="pl-4 space-y-2 mt-2">
                        {item.children.map((child) => (
                          <Link key={child.name} to={createPageUrl(child.page)} onClick={() => setMobileMenuOpen(false)}
                            className="block py-2 text-sm text-gray-600">{child.name}</Link>
                        ))}
                      </div>
                    )}
                  </div>
